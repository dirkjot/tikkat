...
def generate_updates_for_states(slackclient, states=all_states, count=100, oldest=0):
    """ 
    Read the #darkbots channel for update from Airtable and parse them, generate a stream 
    of FieldUpdate records (generator)

    - states: list of states to create records for
    - count: number of messages to retrieve from Slack
...
def airtable_update_completion(updategen, tickets_table):
    ...
    for upd in updategen:
        if upd.table != AIRTABLE_TABLEID:
            print(f"Ignoring updates to old table {upd.table}")
            continue
        ...
        try:
            ...
            print(f"Record with state changes ignored, as not found: {upd.record}")
            continue
